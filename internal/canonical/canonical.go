...
func Normalize(s Snapshot) (Snapshot, error) {
	...
	for i := range s.Tools {
		...
		if err != nil {
			return s, fmt.Errorf("normalize tool %q schema: %w", s.Tools[i].Name, err)
		}
		s.Tools[i].Annotations, err = normalizeJSON(s.Tools[i].Annotations)
		if err != nil {
			return s, fmt.Errorf("normalize tool %q annotations: %w", s.Tools[i].Name, err)
		}
	}
	for i := range s.Prompts {
		...
		if err != nil {
			return s, fmt.Errorf("normalize prompt %q arguments: %w", s.Prompts[i].Name, err)
		}
	}
	var err error
	s.Auth.ProtectedResourceMeta, err = normalizeJSON(s.Auth.ProtectedResourceMeta)
	if err != nil {
		return s, fmt.Errorf("normalize protected resource metadata: %w", err)
	}
	s.Auth.AuthorizationMeta, err = normalizeJSON(s.Auth.AuthorizationMeta)
	if err != nil {
		return s, fmt.Errorf("normalize authorization metadata: %w", err)
	}
	return s, nil
}
