...
func compareTools(a,b []canonical.Tool) []Change {
	...
	for _,x:=range a { am[x.Name]=x }
	for _,x:=range b { bm[x.Name]=x }
	var out []Change
	for n,x:=range am {
		y,ok:=bm[n]
		if !ok {
		...
		if x.Description != y.Description {
			sev:=Low
			if sensitiveDescriptionDelta(x.Description,y.Description) { sev=Medium }
...
func classifySchemaChange(before,after json.RawMessage)(string,Severity,string) {
	...
	if ap && bp && ab!=bb {
		if !ab && bb { widen=true } else { narrow=true }
	}
	ar:=stringSet(a["required"])
	br:=stringSet(b["required"])
	if len(ar)>0 || len(br)>0 {
		if setHasRemoved(ar,br) { widen=true }
		if setHasRemoved(br,ar) { narrow=true }
	}
	ae:=stringSet(a["enum"])
	be:=stringSet(b["enum"])
	if len(ae)>0 && len(be)>0 {
		if isStrictSuperset(be,ae) { widen=true }
		if isStrictSuperset(ae,be) { narrow=true }
	}
	if widen { return "widened",High,"tool input schema widened accepted authority/capability" }
	if narrow { return "narrowed",Medium,"tool input schema narrowed accepted input" }
...
func stringSet(v any) map[string]struct{} {
	...
	if !ok { return out }
	for _,x:=range xs {
		if s,ok:=x.(string);ok { out[s]=struct{}{} }
	}
	return out
}
...
func compareResources(a,b []canonical.Resource) []Change {
	...
	for _,x:=range a { am[key(x)]=x }
	for _,x:=range b { bm[key(x)]=x }
	var out []Change
	for n:=range am {
		if _,ok:=bm[n];!ok { out=append(out,Change{Low,"capability","resources."+n,"removed",n,nil,"resource removed"}) }
	}
	for n:=range bm {
		if _,ok:=am[n];!ok { out=append(out,Change{Medium,"capability","resources."+n,"added",nil,n,"new resource exposed"}) }
	}
	return out
}
...
func comparePrompts(a,b []canonical.Prompt) []Change {
	...
	for n:=range bm {
		if _,ok:=am[n];!ok { out=append(out,Change{Medium,"capability","prompts."+n,"added",nil,n,"new prompt exposed"}) }
	}
	return out
}
...
func rawValue(a json.RawMessage) any {
	if len(bytes.TrimSpace(a))==0 { return nil }
	var v any
	if json.Unmarshal(a,&v)==nil { return v }
	return string(a)
}
...
func risk(score int) string {
	switch {
	case score>=70: return "CRITICAL"
	case score>=35: return "HIGH"
