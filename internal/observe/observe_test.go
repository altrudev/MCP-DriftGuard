...
func TestModernDiscoveryAndPagination(t *testing.T) {
	...
	if s.Server.ProtocolVersion != ModernProtocolVersion { t.Fatalf("version=%q",s.Server.ProtocolVersion) }
	if s.Server.Name != "fixture" { t.Fatalf("name=%q",s.Server.Name) }
	if len(s.Tools) != 2 || toolPages != 2 { t.Fatalf("tools=%d pages=%d",len(s.Tools),toolPages) }
}
...
func TestAuthMetadataPathInsertion(t *testing.T) {
	...
	if err == nil { t.Fatal("expected unauthorized probe error") }
